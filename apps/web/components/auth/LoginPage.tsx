...
export default function LoginPage() {
  ...
  const handleSocialAuth = (provider: string) => {
    window.location.href = `${process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:8000'}/auth/${provider}/redirect`;
  };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-8 space-y-6">
          <div className="flex gap-3 bg-[#f8edeb] p-1.5 rounded-2xl">
            <Link href="/login" className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-white text-[#285301] shadow-sm text-center">
              Sign in
            </Link>
            <Link href="/register" className="flex-1 py-2.5 rounded-xl text-sm font-medium text-gray-600 text-center">
              Sign up
            </Link>
          </div>
          ...
          <div className="text-center">
            <Link href="/forgot-password" className="text-sm text-[#d45d78] hover:underline">
              Forgot password?
            </Link>
          </div>
