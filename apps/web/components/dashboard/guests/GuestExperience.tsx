                    ...
                    {['live', 'hidden', 'setup'].map((status) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={saving}
                          onClick={() => {
                            if (editingModule && (status === 'live' || status === 'hidden')) {
                              toggleModule(editingModule, status as 'live' | 'hidden');
                            }
                          }}
                          className={`px-4 py-2 rounded-full text-[12px] font-medium transition-all disabled:opacity-60 ${badge.color}`}
                        >
                          {badge.label}
                        </button>
                      );
