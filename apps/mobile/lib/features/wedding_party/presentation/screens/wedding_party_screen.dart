...
import '../../../guests/presentation/screens/guests_screen.dart'
    show AddHotelModal, AddTransportModal;
import '../../../home/presentation/providers/home_provider.dart';
import '../providers/wedding_party_provider.dart';

String _fullName(Map<String, dynamic> g) =>
...
String? _idKey(dynamic value) => _asIntId(value)?.toString();
...
String _formatDate(String? iso) {
  return udo_dates.formatApiDateWithWeekday(iso);
}
...
Widget _emptyBox(String message, {IconData icon = Icons.inbox_outlined}) =>
    Center(
      child: Padding(
        padding: const EdgeInsets.all(24),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
...
class WeddingPartyScreen extends ConsumerStatefulWidget {
  ...
  const WeddingPartyScreen({super.key, this.initialTab});
  @override
  ConsumerState<WeddingPartyScreen> createState() => _WeddingPartyScreenState();
}

class _WeddingPartyScreenState extends ConsumerState<WeddingPartyScreen>
  ...
  void initState() {
    ...
    _tabs = TabController(
      length: 7,
      vsync: this,
      initialIndex: _weddingPartyInitialTab(widget.initialTab),
    );
    _tabs.addListener(() {
                  ...
                  child: TabBarView(
                    ...
                    children: [
                      _OverviewTab(
                          state: state,
                          onPersonTap: (id) =>
                              _showPersonDetail(context, state, id),
                          onTabJump: _tabs.animateTo),
                      _PeopleTab(
                          state: state,
                          onPersonTap: (id) =>
                              _showPersonDetail(context, state, id)),
    ...
    showModalBottomSheet(
      ...
      builder: (sheetContext) => SafeArea(
        child: Wrap(children: [
          ...
          ListTile(
            leading:
                const Icon(Icons.person_add_outlined, color: AppTheme.udoGreen),
          ...
          ListTile(
            leading:
                const Icon(Icons.checklist_outlined, color: AppTheme.udoGreen),
            ...
            onTap: () {
              ...
              showModalBottomSheet(
                ...
                builder: (_) => _ResponsibilityEditorSheet(
                    task: null, members: state.members),
              );
            },
          ),
          ListTile(
              ...
              showModalBottomSheet(
                ...
                builder: (_) => BuzzComposerSheet(
                  ...
                  onSend: (body, channel, urgent) => ref
                      ...
                      .sendBuzz(body: body, channel: channel, urgent: urgent),
                ),
              );
            },
          ),
          ListTile(
            ...
            onTap: () async {
              ...
              if (picked == null || picked.bytes == null || !context.mounted)
                return;
              final ok = await ref
                  .read(weddingPartyProvider.notifier)
                  .uploadFile(picked.bytes!, picked.name, category: 'file');
  ...
  void _showPersonDetail(
      BuildContext context, WeddingPartyState state, String id) {
    final person = state.members
        .firstWhere((m) => m['id'].toString() == id, orElse: () => {});
    if (person.isEmpty) return;
    final responsibilities = state.responsibilities
        .where((r) => r['guest_id']?.toString() == id)
        .toList();
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      ...
      builder: (_) => _PersonDetailSheet(
          person: person, responsibilities: responsibilities),
    );
  }
    ...
    showModalBottomSheet(
      ...
      builder: (sheetContext) => SafeArea(
        child: Wrap(children: [
          const Padding(
              padding: EdgeInsets.fromLTRB(20, 16, 20, 4),
              child: Text('Add to wedding party',
                  style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600))),
          ListTile(
            ...
            subtitle: const Text(
                'Reuses their name, email and phone — no duplicate record.',
        ...
        onAdd: (
          ...
          if (!ok) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text("Couldn't add this person. Try again.")));
          }
        },
      ),
    );
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _LinkExistingGuestSheet(
        onLink: (guestId, role) async {
          final ok = await ref
              .read(weddingPartyProvider.notifier)
              .linkExistingGuest(guestId, role: role);
          if (!context.mounted) return;
          if (!ok) {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text("Couldn't add this person. Try again.")));
...
int _weddingPartyInitialTab(String? tab) {
  switch (tab) {
    case 'people':
      return 1;
...
class _Header extends StatelessWidget {
  ...
  const _Header(
      {required this.tabs, required this.count, required this.onRefresh});
...
class _OverviewTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final openResponsibilities =
        state.responsibilities.where((r) => r['status'] != 'done').length;
    ...
    final topPriorities =
        state.responsibilities.where((r) => r['status'] != 'done').toList()
          ..sort((a, b) {
            final da = dueOf(a);
            final db = dueOf(b);
    ...
    if (state.membersError != null) {
      return _errorBox(
          "Couldn't load your wedding party.", state.membersError!);
    }
...
class _MiniStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Expanded(
        ...
        Text(label,
            style:
                const TextStyle(fontSize: 10, color: AppTheme.udoTextSecondary),
            textAlign: TextAlign.center),
      ]));
}
...
class _PeopleTab extends ConsumerWidget {
  ...
  Color _attireColor(String? s) => s == 'ready'
  ...
  Color _rehearsalColor(String? s) => s == 'confirmed'
      ? const Color(0xFF22C55E)
      : s == 'declined'
          ? AppTheme.udoCrimson
          : Colors.orange;
    ...
    return ListView(
      ...
      children: [
        if (state.members.isEmpty)
          _emptyBox(
              'No one added to your wedding party yet. Tap + to add someone.',
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Column(
                        ...
                        Text(
                            (m['wedding_party_role'] as String?) ??
                                'Wedding party',
                            style: const TextStyle(
                                fontSize: 13,
                        ...
                        const SizedBox(height: 6),
                        Row(children: [
                          _MiniChip(_attireLabel(m['attire_status'] as String?),
                              _attireColor(m['attire_status'] as String?)),
                          const SizedBox(width: 6),
                          _MiniChip(
                              _rehearsalLabel(m['rehearsal_status'] as String?),
                              _rehearsalColor(
                                  m['rehearsal_status'] as String?)),
                        ]),
                      ])),
                  GestureDetector(
                      onTap: () => _launchTel(context, m['phone'] as String?),
                      child: Icon(Icons.phone_outlined,
                          color: (m['phone'] as String?)?.isNotEmpty == true
                              ? AppTheme.udoGreen
                              : AppTheme.udoBorder,
                          size: 20)),
                  const SizedBox(width: 12),
                  GestureDetector(
                      onTap: () => _launchSms(context, m['phone'] as String?),
        ...
        Row(children: [
          const Expanded(
              child: Text('Additional emergency contacts',
        ...
        if (state.emergencyError != null)
          ...
          _emptyBox(
              'No additional contacts yet — add parents, coordinator, or venue.',
              icon: Icons.contact_phone_outlined)
        else
          for (final c in state.emergencyContacts)
            Container(
              ...
              child: Row(children: [
                ...
                GestureDetector(
                    onTap: () => _launchTel(context, c['phone'] as String?),
                ...
                GestureDetector(
                  onTap: () => ref
                      .read(weddingPartyProvider.notifier)
                      .removeEmergencyContact(c['id'] as int),
...
class _ResponsibilitiesTab extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_ResponsibilitiesTab> createState() =>
      _ResponsibilitiesTabState();
}
...
class _ResponsibilitiesTabState extends ConsumerState<_ResponsibilitiesTab> {
  ...
  String _guestName(WeddingPartyState state, dynamic guestId) {
    ...
    final g = state.members.firstWhere(
        (m) => m['id'].toString() == guestId.toString(),
        orElse: () => {});
  ...
  Future<void> _exportTasks(
      BuildContext context, WeddingPartyState state) async {
    ...
    await Share.share(buffer.toString(),
        subject: 'Wedding party responsibilities');
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (state.responsibilitiesError != null) {
      return _errorBox(
          "Couldn't load responsibilities.", state.responsibilitiesError!);
    }
      ...
      if (_selected.isNotEmpty)
        _BulkActionBar(
          ...
          onMarkComplete: () async {
            ...
            await ref
                .read(weddingPartyProvider.notifier)
                .bulkUpdateResponsibilities(ids, {'status': 'done'});
          },
        ...
        child: ListView(
          ...
          children: [
            if (state.responsibilities.isNotEmpty) ...[
              ...
              Wrap(spacing: 8, runSpacing: 8, children: [
                FilterChip(
                  label: Text('All (${state.responsibilities.length})',
                ...
                for (final entry in _kResponsibilityFilters.entries)
                  FilterChip(
                    label: Text(
                        '${entry.value} (${buckets[entry.key]!.length})',
                        style: const TextStyle(fontSize: 12)),
                    selected: _statusFilter == entry.key,
                    onSelected: (sel) =>
                        setState(() => _statusFilter = sel ? entry.key : null),
                    selectedColor: AppTheme.udoGreen.withValues(alpha: 0.15),
                  ),
              ]),
              const SizedBox(height: 12),
              Row(children: [
                Expanded(
                  ...
                  label: const Text('Create group task',
                      style: TextStyle(fontSize: 12)),
                )),
                const SizedBox(width: 8),
                Expanded(
            ...
            Row(children: [
              const Expanded(
                  child: Text('Wedding party responsibilities',
                      style: TextStyle(
                          fontSize: 15, fontWeight: FontWeight.w500))),
              TextButton.icon(
            ...
            if (state.responsibilities.isEmpty)
              _emptyBox('No responsibilities yet. Tap Add to assign one.',
              ...
              for (final section in [
                ...
                if (groups[section]!.isNotEmpty) ...[
                  ...
                  for (final task in groups[section]!)
                    _ResponsibilityCard(
                      task: task,
  ...
  Future<void> _sendReminderToAssignees(
      BuildContext context, WeddingPartyState state) async {
    final selectedTasks = state.responsibilities
        .where((r) => _selected.contains(r['id'] as int))
        .toList();
    final titles = selectedTasks
  ...
  void _openEditor(BuildContext context, Map<String, dynamic>? task) {
    showModalBottomSheet(
      ...
      shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
      builder: (_) =>
          _ResponsibilityEditorSheet(task: task, members: widget.state.members),
    );
  }
}
    ...
    return Container(
      ...
      child: Row(children: [
        SizedBox(
            ...
            height: 72,
            child: CustomPaint(
                painter: _ResponsibilityDonutPainter(counts: counts))),
        ...
        Expanded(
          ...
          for (final key in const [
            ...
            Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(width: 6),
              Text('${_kResponsibilityFilters[key]} (${counts[key]})',
                  style: const TextStyle(
...
class _ResponsibilityDonutPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _ResponsibilityDonutPainter oldDelegate) =>
...
class _GroupTaskSheetState extends ConsumerState<_GroupTaskSheet> {
  ...
  Future<void> _save() async {
    ...
    final matching =
        widget.members.where((m) => m['wedding_party_role'] == _role).toList();
    final notifier = ref.read(weddingPartyProvider.notifier);
    var allOk = true;
    for (final m in matching) {
      final guestId = _asIntId(m['id']);
      if (guestId == null) {
        allOk = false;
        continue;
      }
    ...
    if (allOk) {
      ...
      setState(() {
        _saving = false;
        _error = "Couldn't create tasks for everyone. Try again.";
      });
    }
  }
        ...
        child: Padding(
          ...
          child: SingleChildScrollView(
              child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                const Text('Create group task',
                    style:
                        TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
                const SizedBox(height: 4),
                const Text(
                    'Creates the same task for everyone sharing a role — e.g. all Groomsmen.',
                    style: TextStyle(
                        fontSize: 12, color: AppTheme.udoTextSecondary)),
                const SizedBox(height: 16),
                ...
                _SheetField('Description', _description,
                    maxLines: 2, hint: 'Optional'),
                const SizedBox(height: 14),
                const Text('Group',
                    style:
                ...
                if (roles.isEmpty)
                  const Text(
                      'No shared roles yet — add roles to members first.',
                      style: TextStyle(
                          fontSize: 12, color: AppTheme.udoTextSecondary))
                else
                  DropdownButtonFormField<String>(
                    ...
                    items: roles
                        .map((r) => DropdownMenuItem(
                            value: r,
                            child: Text(
                                '$r (${widget.members.where((m) => m['wedding_party_role'] == r).length})')))
                        .toList(),
            ...
            border: Border.all(
                color: selected
                    ...
                    : (task['status'] == 'in_progress'
                        ? AppTheme.udoGreen.withValues(alpha: 0.4)
                        : AppTheme.udoBorder)),
          ),
          child:
            ...
            Row(children: [
              ...
              if (task['due_date'] != null) ...[
                ...
                const Icon(Icons.schedule,
                    size: 14, color: AppTheme.udoTextSecondary),
                const SizedBox(width: 4),
                Text(_formatDate(task['due_date'] as String?),
                    style: const TextStyle(
                        fontSize: 12, color: AppTheme.udoTextSecondary)),
              ],
            ]),
...
class _ResponsibilityEditorSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_ResponsibilityEditorSheet> createState() =>
      _ResponsibilityEditorSheetState();
}

class _ResponsibilityEditorSheetState
    extends ConsumerState<_ResponsibilityEditorSheet> {
  ...
  Future<void> _save() async {
    ...
    bool ok;
    if (_isEdit) {
      final taskId = _asIntId(widget.task!['id']);
      if (taskId == null) {
        setState(() {
          _saving = false;
          _error =
              "Couldn't identify this responsibility. Try opening it again.";
        });
        return;
      }
      final guestId = _guestId == null ? null : _asIntId(_guestId);
      if (_guestId != null && guestId == null) {
        setState(() {
          _saving = false;
          _error = "Couldn't identify the selected person. Pick them again.";
        });
        return;
      }
      ok = await notifier.updateResponsibility(taskId, {
        'title': _title.text.trim(),
        'description': _description.text.trim(),
        'guest_id': guestId,
        'status': _status,
    ...
    if (ok) {
      ...
      setState(() {
        ...
        _error = _isEdit
            ...
            : "Couldn't create tasks for everyone. Try again.";
      });
    }
  }
  ...
  Future<void> _delete() async {
    ...
    await ref
        .read(weddingPartyProvider.notifier)
        .deleteResponsibility(widget.task!['id'] as int);
  }
                    ...
                    children: [
                      ...
                      if (_isEdit)
                        DropdownButtonFormField<String?>(
                          ...
                          items: [
                            ...
                            ...widget.members.map((m) {
                              final id = _idKey(m['id']);
                              if (id == null) return null;
                              return DropdownMenuItem<String?>(
                                  value: id, child: Text(_fullName(m)));
                        ...
                        Wrap(spacing: 8, runSpacing: 8, children: [
                          for (final m in widget.members)
                            FilterChip(
                              ...
                              selected: _assigneeIds.contains(_idKey(m['id'])),
                        ...
                        if (_assigneeIds.length > 1) ...[
                          const SizedBox(height: 6),
                          Text(
                              'Creates one task for each of the ${_assigneeIds.length} people selected.',
                              style: const TextStyle(
                                  fontSize: 12,
                                  color: AppTheme.udoTextSecondary)),
                        ] else if (_assigneeIds.isEmpty) ...[
                          const SizedBox(height: 6),
                          const Text(
                              'No one selected — this task will be unassigned.',
                              style: TextStyle(
                                  fontSize: 12,
                                  color: AppTheme.udoTextSecondary)),
                        ],
...
InputDecoration _sheetDecoration() => InputDecoration(
    filled: true,
...
Widget _responsibilityTitlePicker({
  ...
  return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    DropdownButtonFormField<String>(
      ...
      items: [
        for (final t in titleChoices)
          DropdownMenuItem(
              value: t, child: Text(t, overflow: TextOverflow.ellipsis)),
        const DropdownMenuItem(
            value: _customResponsibilityTitleSentinel,
...
const _kBuzzTemplates = {
  'Morning check-in':
      "Good morning! Just checking in — let us know if you need anything today.",
  'Rehearsal reminder':
      'A quick reminder that our rehearsal is scheduled soon. Please confirm that you will be there.',
  'Attire reminder':
      "Reminder: please confirm your attire status if you haven't already.",
  'Travel reminder':
      "Reminder: please share your travel details if you haven't already, so we can plan pickups and hotel rooms.",
  'Thank you message':
      "Thank you so much for everything you're doing to help make this day special. We're so grateful for you.",
};
...
class _BuzzesTabState extends ConsumerState<_BuzzesTab> {
  ...
  bool _isThisWeek(DateTime? d) =>
      d != null &&
      !d.isBefore(DateTime.now().subtract(const Duration(days: 7)));
  ...
  int _successfulSends(Map<String, dynamic>? summary) =>
      _summaryCount(summary, 'sent') +
      _summaryCount(summary, 'delivered') +
      _summaryCount(summary, 'opened');

  @override
  Widget build(BuildContext context) {
    final state = widget.state;
    final sentThisWeek = state.buzzes
        .where((m) =>
            _isThisWeek(DateTime.tryParse(m['sent_at'] as String? ?? '')))
        .toList();
    final peopleReached = sentThisWeek.fold<int>(
        0, (sum, m) => sum + ((m['recipient_count'] as num?)?.toInt() ?? 0));
    var successfulSum = 0;
    var attemptedSum = 0;
    for (final m in sentThisWeek) {
      final summary = m['delivery_summary'] as Map<String, dynamic>?;
      successfulSum += _successfulSends(summary);
      attemptedSum += (summary?['total'] as num?)?.toInt() ?? 0;
    }
    final sendRate = attemptedSum > 0
        ? ((successfulSum / attemptedSum) * 100).round()
        : null;
    ...
    return ListView(
      ...
      children: [
        if (state.buzzes.isNotEmpty) ...[
          Container(
            ...
            child: Row(children: [
              ...
              _MiniStat(sendRate != null ? '$sendRate%' : '—', 'Sent\nrate',
                  const Color(0xFF22C55E)),
            ]),
          ),
          const SizedBox(height: 16),
          ...
          for (final entry in _kBuzzTemplates.entries)
            ActionChip(
              ...
              onPressed: state.members.isEmpty
                  ? null
                  : () => _openComposer(context, ref, initialBody: entry.value),
        ...
        ElevatedButton.icon(
          onPressed:
              state.members.isEmpty ? null : () => _openComposer(context, ref),
        ...
        if (state.members.isEmpty)
          const Padding(
              ...
              child: Text('Add someone to your wedding party first.',
                  style: TextStyle(
                      fontSize: 12, color: AppTheme.udoTextSecondary))),
        const SizedBox(height: 10),
        OutlinedButton.icon(
          onPressed: state.members.isEmpty
              ? null
              : () => _openComposer(context, ref, initialUrgent: true),
        ...
        if (state.buzzesError != null)
          ...
          const Padding(
              ...
              child: Text('No messages in this range.',
                  style: TextStyle(
                      fontSize: 13, color: AppTheme.udoTextSecondary)))
        else
          for (final msg in visibleBuzzes)
            Container(
              margin: const EdgeInsets.only(bottom: 8),
              padding: const EdgeInsets.all(14),
              decoration: BoxDecoration(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    Row(children: [
                      Text(
                          'Via ${_kBuzzChannelLabels[msg['channel']] ?? msg['channel']} · ${msg['recipient_count'] ?? 0} recipient${(msg['recipient_count'] ?? 0) == 1 ? '' : 's'}',
                          style: const TextStyle(
                              fontSize: 11, color: AppTheme.udoTextSecondary)),
                      const Spacer(),
                      ...
                      _DeliveryBadge(
                          msg['delivery_summary'] as Map<String, dynamic>?),
                    ]),
                    const SizedBox(height: 4),
                    Text(_formatDate(msg['sent_at'] as String?),
                        style: const TextStyle(
                            fontSize: 11, color: AppTheme.udoTextSecondary)),
                  ]),
            ),
  ...
  void _openComposer(BuildContext context, WidgetRef ref,
      {String initialBody = '', bool initialUrgent = false}) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      shape: const RoundedRectangleBorder(
...
class _DeliveryBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (total == 0) {
      ...
      label = delivered > 0 ? 'Delivered' : 'Sent';
                    ...
                    children: [
                      ...
                      Row(children: [
                        ...
                        Switch(
                            ...
                            activeColor: AppTheme.udoCrimson),
                      ]),
                      if (_urgent)
                        Container(
                          ...
                          decoration: BoxDecoration(
                              color:
                                  AppTheme.udoCrimson.withValues(alpha: 0.08),
                      ...
                      Wrap(spacing: 8, children: [
                        for (final entry in _kBuzzChannelLabels.entries)
                          ChoiceChip(
                            ...
                            onSelected: (_) =>
                                setState(() => _channel = entry.key),
                            selectedColor: AppTheme.udoGreen,
                            labelStyle: TextStyle(
                                color: _channel == entry.key
                                    ? Colors.white
                          ...
                          child: Row(
                              ...
                              children: [
                                const Icon(Icons.info_outline,
                                    size: 15, color: AppTheme.udoGreen),
                                const SizedBox(width: 8),
                                Expanded(
                                  child: Text(
                                    guestPortalUrl != null &&
                                            guestPortalUrl.isNotEmpty
  ...
  Future<void> _submit() async {
    ...
    if (_urgent) {
      final confirmed = await showDialog<bool>(
        ...
        builder: (dialogContext) => AlertDialog(
          ...
          content: Text(
              'You are about to send an urgent message to ${widget.recipientCount} ${widget.recipientCount == 1 ? 'person' : 'people'} via ${_kBuzzChannelLabels[_channel]}.'),
          actions: [
            TextButton(
    ...
    if (urgent) {
      ...
      messenger.showSnackBar(
          const SnackBar(content: Text('Urgent message is sending...')));
    }
    final ok = await widget.onSend(body, channel, urgent);
    if (!mounted && !urgent) return;
    if (ok) {
      if (!urgent) navigator.pop();
      messenger.showSnackBar(SnackBar(
          content: Text(urgent ? 'Urgent message sent.' : 'Message sent.')));
    } else {
      if (!urgent) setState(() => _sending = false);
      messenger.showSnackBar(const SnackBar(
          content: Text("Couldn't send that message. Try again.")));
    }
  }
...
class _TravelTab extends ConsumerWidget {
  ...
  String _travelStatus(Map<String, dynamic> m) {
    ...
    if (m['hotel_assignment_id'] != null &&
        m['transport_assignment_id'] != null) return 'arranged';
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    if (state.travelError != null)
      return _errorBox("Couldn't load travel details.", state.travelError!);

    final needsInfo =
        state.members.where((m) => _travelStatus(m) == 'needs-info').toList();
    final travelling =
        state.members.where((m) => m['travel_required'] == true).toList();
    final local =
        state.members.where((m) => m['travel_required'] != true).length;
    final arranged =
        travelling.where((m) => _travelStatus(m) == 'arranged').length;
    final coveragePct = travelling.isNotEmpty
        ? ((arranged / travelling.length) * 100).round()
        : null;
    ...
    final sortedArrivalKeys = arrivalGroups.keys.toList()
      ..sort((a, b) => arrivalGroups[a]!
          .first['arrival_date']
          .toString()
          .compareTo(arrivalGroups[b]!.first['arrival_date'].toString()));
      ...
      children: [
        Container(
          ...
          child: Row(children: [
            ...
            _MiniStat(
                '${needsInfo.length}', 'Details\npending', AppTheme.udoCrimson),
            _MiniStat(coveragePct != null ? '$coveragePct%' : '—',
            ...
            child:
              ...
              for (final key in sortedArrivalKeys)
                Padding(
                  ...
                  child: Text('${arrivalGroups[key]!.length} arriving $key',
                      style: const TextStyle(fontSize: 13)),
                ),
            ]),
          ),
        ...
        if (needsInfo.isNotEmpty) ...[
          Container(
            ...
            decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(16),
                border: Border.all(
                    color: AppTheme.udoCrimson.withValues(alpha: 0.2))),
            child:
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                const Icon(Icons.info_outline,
                ...
                Expanded(
                    child: Text(
                        '${needsInfo.length} ${needsInfo.length == 1 ? 'person is' : 'people are'} missing travel details',
                        style: const TextStyle(
                            fontSize: 13,
                            fontWeight: FontWeight.w500,
                            color: AppTheme.udoCrimson))),
        ...
        Container(
          ...
          child:
              ...
              TextButton.icon(
                onPressed: () => showModalBottomSheet(
                  ...
                  shape: const RoundedRectangleBorder(
                      borderRadius:
                          BorderRadius.vertical(top: Radius.circular(24))),
                  builder: (_) => AddHotelModal(
                      notifier: ref.read(logisticsProvider.notifier)),
                  ...
                  // manual pull-to-refresh.
                    ...
                    (_) => ref.read(weddingPartyProvider.notifier).refresh()),
                icon: const Icon(Icons.add, size: 16),
                label: const Text('Add', style: TextStyle(fontSize: 13)),
              ),
            ]),
            ...
            if (state.accommodations.isEmpty)
              const Text('No accommodation options added yet.',
                  style:
                      TextStyle(fontSize: 12, color: AppTheme.udoTextSecondary))
            else
              for (final a in state.accommodations)
                Container(
                      ...
                      children: [
                        Text(a['name'] as String? ?? '',
                            style: const TextStyle(
                                fontSize: 13, fontWeight: FontWeight.w500)),
                        Text(
                            '${a['total_rooms_blocked'] ?? 0} rooms blocked · ${a['rooms_assigned'] ?? 0} assigned',
                            style: const TextStyle(
                                fontSize: 12,
                                color: AppTheme.udoTextSecondary)),
                      ]),
        ...
        Container(
          ...
          child:
            ...
            if (state.transportGroups.isEmpty)
              const Text('No transport groups added yet.',
                  style:
                      TextStyle(fontSize: 12, color: AppTheme.udoTextSecondary))
            else
              for (final t in state.transportGroups)
                Container(
                  margin: const EdgeInsets.only(bottom: 6),
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                      ...
                      children: [
                        Text(t['name'] as String? ?? '',
                            style: const TextStyle(
                                fontSize: 13, fontWeight: FontWeight.w500)),
                        Text(
                            '${t['pickup_location'] ?? '—'} → ${t['dropoff_location'] ?? '—'} · ${(t['assignments'] as List?)?.length ?? 0} assigned',
                            style: const TextStyle(
                                fontSize: 12,
                                color: AppTheme.udoTextSecondary)),
                      ]),
        ...
        const SizedBox(height: 12),
        const Text('Travel status by member',
            style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500)),
        const SizedBox(height: 8),
        if (state.members.isEmpty)
  ...
  void _requestTravelDetails(BuildContext context, WidgetRef ref,
      List<Map<String, dynamic>> needsInfo) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => BuzzComposerSheet(
        initialBody:
            "Hi everyone — we're missing travel details for $names. Please share your arrival date, accommodation and transport needs so we can finalise arrangements.",
        recipientCount: state.members.length,
        onSend: (body, channel, urgent) => ref
            .read(weddingPartyProvider.notifier)
            .sendBuzz(body: body, channel: channel, urgent: urgent),
...
class _RehearsalTab extends ConsumerWidget {
  ...
  void _openAddRehearsal(BuildContext context,
    ...
    showModalBottomSheet(
      ...
      shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
      builder: (_) =>
          _AddRehearsalSheet(members: state.members, rehearsal: rehearsal),
    );
  }
  ...
  Future<void> _sendReminder(BuildContext context, WidgetRef ref) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(ok
            ? 'Reminder sent to your wedding party.'
            : "Couldn't send that reminder.")));
  }

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    if (state.rehearsalsError != null)
      return _errorBox("Couldn't load the rehearsal.", state.rehearsalsError!);
      ...
      return ListView(
        ...
        children: [
          Container(
            ...
            child:
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('No rehearsal scheduled yet',
                  style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500)),
              const SizedBox(height: 4),
              const Text(
                  'Add the details for your rehearsal so your wedding party knows when and where to be.',
                  style: TextStyle(
                      fontSize: 12, color: AppTheme.udoTextSecondary)),
              const SizedBox(height: 10),
              ElevatedButton.icon(
    ...
    final color =
        _parseHexColor(rehearsal['color'] as String?) ?? AppTheme.udoGreen;
    final attendeeIds = (rehearsal['attendee_guest_ids'] as List?)
        ?.map((id) => id.toString())
        .toSet();
    final attendance = (attendeeIds != null && attendeeIds.isNotEmpty)
        ? state.members
            .where((m) => attendeeIds.contains(m['id'].toString()))
            .toList()
        : state.members;

    final invited = attendance.length;
    final responded = attendance
        .where(
            (m) => (m['rehearsal_status'] as String? ?? 'pending') != 'pending')
        .length;
    final pending = invited - responded;
    final unavailable =
        attendance.where((m) => m['rehearsal_status'] == 'declined').length;

    final scheduleItems = (rehearsal['schedule_items'] as List?)
      ...
      children: [
        ...
        Container(
          ...
          child:
            ...
            Row(children: [
              const Expanded(
                  child: Text('Rehearsal Overview',
                      style: TextStyle(
                          fontSize: 15, fontWeight: FontWeight.w500))),
              IconButton(
                onPressed: () =>
                    _openAddRehearsal(context, rehearsal: rehearsal),
            ...
            Wrap(spacing: 12, runSpacing: 4, children: [
              _InfoChip(Icons.calendar_today_outlined,
                  _formatDate(rehearsal['event_date'] as String?)),
              _InfoChip(Icons.schedule_outlined,
                  '${_formatTime(rehearsal['start_time'] as String?)}${(rehearsal['end_time'] as String?)?.isNotEmpty == true ? ' – ${_formatTime(rehearsal['end_time'] as String?)}' : ''}'),
              if ((rehearsal['location'] as String?)?.isNotEmpty == true)
                _InfoChip(
                    Icons.place_outlined, rehearsal['location'] as String),
            ]),
            const SizedBox(height: 16),
            Row(children: [
              Expanded(
                  child: _RehearsalStat(
                      color: color,
        ...
        Container(
          ...
          child:
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Rehearsal Details',
                style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500)),
            const SizedBox(height: 10),
            if ((rehearsal['location'] as String?)?.isNotEmpty == true)
              _DetailRow(Icons.place_outlined, 'Location',
                  rehearsal['location'] as String),
            ...
            if ((rehearsal['dress_code'] as String?)?.isNotEmpty == true)
              _DetailRow(Icons.checkroom_outlined, 'Attire',
                  rehearsal['dress_code'] as String),
            if ((rehearsal['notes'] as String?)?.isNotEmpty == true)
              _DetailRow(
                  Icons.notes_outlined, 'Notes', rehearsal['notes'] as String,
                  isLast: true),
          ]),
        ),
                ...
                Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Column(children: [
                    ...
                    if (i != scheduleItems.length - 1)
                      Container(
                          width: 2, height: 44, color: AppTheme.udoBorder),
                  ]),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Padding(
                      ...
                      child: Column(
                          ...
                          children: [
                            ...
                            Text(scheduleItems[i]['title'] as String? ?? '',
                                style: const TextStyle(
                                    fontSize: 13, fontWeight: FontWeight.w500)),
                            if ((scheduleItems[i]['description'] as String?)
                                    ?.isNotEmpty ==
                                true)
                              Text(scheduleItems[i]['description'] as String,
                                  style: const TextStyle(
                                      fontSize: 12,
                                      color: AppTheme.udoTextSecondary)),
                          ]),
        ...
        Row(children: [
          ...
          TextButton.icon(
            ...
            label:
                const Text('Manage Attendees', style: TextStyle(fontSize: 12)),
          ),
        ]),
        const SizedBox(height: 8),
  ...
  Future<void> _cycleStatus(WidgetRef ref, Map<String, dynamic> m) async {
    ...
    await ref
        .read(weddingPartyProvider.notifier)
        .updateMember(m['id'] as int, {'rehearsal_status': next});
  }
}
  ...
  Widget build(BuildContext context) => Column(children: [
        ...
        GestureDetector(
          ...
          child: Text(action,
              style: TextStyle(
                  fontSize: 10,
                  color: onAction != null ? color : AppTheme.udoTextSecondary,
                  fontWeight: FontWeight.w500),
              textAlign: TextAlign.center),
        ),
      ]);
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  shape: BoxShape.circle,
                  border: value == hex
                      ? Border.all(color: AppTheme.udoTextPrimary, width: 2)
                      : null,
                ),
                child: value == hex
...
class _RehearsalScheduleRow {
  ...
  _RehearsalScheduleRow(
      {this.time, String initialTitle = '', String initialDescription = ''})
...
class _AddRehearsalSheetState extends ConsumerState<_AddRehearsalSheet> {
  ...
  TimeOfDay? _endTime;
  String? _timezone;
  late final Set<String> _audienceGroups;
  ...
  void initState() {
    ...
    final bringItems =
        (r?['bring_items'] as List?)?.map((e) => e.toString()).toList();
    ...
    final savedAudienceGroups =
        (r?['audience_groups'] as List?)?.map((value) => value.toString());
    _audienceGroups =
        (savedAudienceGroups != null && savedAudienceGroups.isNotEmpty
                ? savedAudienceGroups
                : [r?['audience'] as String? ?? 'wedding_party'])
            .where((value) => _rehearsalAudienceOptions.containsKey(value))
            .toSet();
    if (_audienceGroups.isEmpty) {
      _audienceGroups.add('wedding_party');
    }
    _attendeeIds = (r?['attendee_guest_ids'] as List?)
            ?.map((id) => id.toString())
            .toSet() ??
        {};
    ...
    if (rawSchedule != null) {
      ...
      _scheduleRows = [
        _RehearsalScheduleRow(
            initialTitle: 'Arrival & Check-in',
            initialDescription: 'Please sign in when you arrive'),
        _RehearsalScheduleRow(
            initialTitle: 'Welcome & Instructions',
            initialDescription: 'Overview of the rehearsal flow and key notes'),
        _RehearsalScheduleRow(
            initialTitle: 'Full Processional Run-through',
            initialDescription: 'Walking order and timing'),
        _RehearsalScheduleRow(
            initialTitle: 'Ceremony Run-through',
            initialDescription: 'Vows, readings and special moments'),
      ];
    }
  }
  ...
  void _addScheduleRow() =>
      setState(() => _scheduleRows.add(_RehearsalScheduleRow()));
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      'audience':
          _audienceGroups.contains('all') ? 'all' : _audienceGroups.first,
      'audience_groups': _audienceGroups.toList(),
      'attendee_guest_ids': _attendeeIds.map(int.parse).toList(),
      if (_dressCode != null) 'dress_code': _dressCode,
      'bring_items': _bringItems.text
          .split(',')
          .map((s) => s.trim())
      ...
      'schedule_items': _scheduleRows
          ...
          .map((row) => {
                ...
                if (row.description.text.trim().isNotEmpty)
        ...
        child: Container(
          ...
          child: Row(children: [
            ...
            Expanded(
                child: Text(
                    _eventDate != null
                        ? _formatDate(_eventDate!.toIso8601String())
                        : 'Select date',
                    style: const TextStyle(fontSize: 12),
                    overflow: TextOverflow.ellipsis)),
          ]),
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 6),
            Expanded(
                child: Text(value != null ? value.format(context) : label,
                    style: const TextStyle(fontSize: 12),
                    overflow: TextOverflow.ellipsis)),
          ]),
        ),
        ...
        child: DraggableScrollableSheet(
          ...
          builder: (_, ctrl) => Column(children: [
            Container(
              ...
              child: Row(children: [
                Expanded(
                    child: Text(_isEdit ? 'Edit rehearsal' : 'Add Rehearsal',
                        style: const TextStyle(
                            fontSize: 18, fontWeight: FontWeight.w600))),
                IconButton(
                    onPressed: () => Navigator.pop(context),
                    ...
                    children: [
                      const Text('BASIC DETAILS',
                          style: TextStyle(
                              ...
                              letterSpacing: 0.5)),
                      const SizedBox(height: 10),
                      _SheetField('Rehearsal title', _title,
                          hint: 'e.g. Wedding Rehearsal'),
                      const SizedBox(height: 14),
                      const Text('Color',
                          style: TextStyle(
                              fontSize: 14, fontWeight: FontWeight.w500)),
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          hintText:
                              'Add any notes or details about the rehearsal...',
                          hintStyle: const TextStyle(
                              color: AppTheme.udoTextSecondary, fontSize: 13),
                          filled: true,
                          fillColor: AppTheme.udoCardFill,
                      ...
                      Row(children: [
                        ...
                        const SizedBox(width: 8),
                        Expanded(
                            child: _timePickButton('Start time', _startTime,
                                (t) => setState(() => _startTime = t))),
                        const SizedBox(width: 8),
                        Expanded(
                            child: _timePickButton('End time', _endTime,
                        ...
                        items: [
                          for (final tz in _timezoneOptions)
                            DropdownMenuItem(
                                value: tz,
                                child:
                                    Text(tz, overflow: TextOverflow.ellipsis)),
                        ],
                      ...
                      const SizedBox(height: 4),
                      const Text(
                          'Optional run-of-show for the rehearsal itself.',
                          style: TextStyle(
                              fontSize: 12, color: AppTheme.udoTextSecondary)),
                      const SizedBox(height: 10),
                      ...
                      Wrap(spacing: 8, runSpacing: 8, children: [
                        for (final entry in _rehearsalAudienceOptions.entries)
                          FilterChip(
                            label: Text(entry.value,
                                style: const TextStyle(fontSize: 12)),
                            selected: _audienceGroups.contains(entry.key),
                            onSelected: (selected) => setState(() {
                              if (entry.key == 'all' && selected) {
                                _audienceGroups
                                  ..clear()
                                  ..add('all');
                                return;
                              ...
                              if (selected) {
                                ...
                                _audienceGroups.remove(entry.key);
                                if (_audienceGroups.isEmpty) {
                                  _audienceGroups.add('wedding_party');
                                }
                              }
                            ...
                            selectedColor: AppTheme.udoGreen,
                            labelStyle: TextStyle(
                                color: _audienceGroups.contains(entry.key)
                                    ? Colors.white
                                    : AppTheme.udoTextPrimary),
                          ),
                      ]),
                      const SizedBox(height: 14),
                      const Text('Add specific people',
                          style: TextStyle(
                              fontSize: 14, fontWeight: FontWeight.w500)),
                      const SizedBox(height: 4),
                      const Text(
                          'Select from your wedding party or add guests.',
                          style: TextStyle(
                              fontSize: 12, color: AppTheme.udoTextSecondary)),
                      const SizedBox(height: 8),
                      Wrap(spacing: 8, runSpacing: 8, children: [
                      ...
                      Row(
                          ...
                          children: [
                              ...
                              child: Column(
                                  ...
                                  children: [
                                    ...
                                    DropdownButtonFormField<String>(
                                      ...
                                      decoration: _sheetDecoration(),
                                      items: [
                                        for (final d in _dressCodeOptions)
                                          DropdownMenuItem(
                                              value: d,
                                              child: Text(d,
                                                  style: const TextStyle(
                            ...
                            Expanded(
                                child: _SheetField(
                                    'Bring anything?', _bringItems,
                                    hint: 'e.g. Vows, attire, scripts')),
                          ]),
                      const SizedBox(height: 4),
                      const Text('Separate multiple items with commas',
                          style: TextStyle(
                              fontSize: 11, color: AppTheme.udoTextSecondary)),
                      const SizedBox(height: 14),
                      _SheetField('Notes (Optional)', _notes,
                          maxLines: 3, hint: 'Any other notes or reminders...'),
                      const SizedBox(height: 14),
                      Row(children: [
                        Expanded(
                          child: Column(
                              ...
                              children: [
                                const Text('Add to timeline',
                                    style: TextStyle(
                                        ...
                                        fontWeight: FontWeight.w500)),
                                const Text(
                                    'Add this rehearsal to your wedding timeline',
                                    style: TextStyle(
                                        fontSize: 12,
                                        color: AppTheme.udoTextSecondary)),
                              ]),
                      ...
                      icon: _saving
                          ? const SizedBox(
                              ...
                              child: CircularProgressIndicator(
                                  strokeWidth: 2, color: Colors.white))
                          : const Icon(Icons.save_outlined, size: 16),
                      label: Text(_saving ? 'Saving…' : 'Save Rehearsal',
                          style: const TextStyle(fontSize: 13)),
                      style: ElevatedButton.styleFrom(
                          backgroundColor: AppTheme.udoGreen,
                          foregroundColor: Colors.white,
...
class _AddEmergencyContactModal extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_AddEmergencyContactModal> createState() =>
      _AddEmergencyContactModalState();
}

class _AddEmergencyContactModalState
    extends ConsumerState<_AddEmergencyContactModal> {
        ...
        child: Padding(
          ...
          child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                _FieldBox('Relationship', _relationship,
                    hint: 'e.g. Venue coordinator'),
                const SizedBox(height: 12),
...
class _FilesSpeechesTabState extends ConsumerState<_FilesSpeechesTab> {
  ...
  Future<void> _upload(String category) async {
    ...
    setState(() {
      if (category == 'speech')
        _uploadingSpeech = true;
      else
        _uploadingFile = true;
    });
    final ok = await ref
        .read(weddingPartyProvider.notifier)
        .uploadFile(picked.bytes!, picked.name, category: category);
    if (!mounted) return;
    setState(() {
      if (category == 'speech')
        _uploadingSpeech = false;
      else
        _uploadingFile = false;
    });
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content:
            Text(ok ? 'Uploaded.' : "Couldn't upload that file. Try again.")));
  }
  ...
  Future<void> _download(Map<String, dynamic> file) async {
    ...
    final absolute =
        url.startsWith('http') ? url : '${AppConstants.apiOrigin}$url';
    final ok = await launchUrl(Uri.parse(absolute),
  ...
  Widget build(BuildContext context) {
    ...
    if (state.filesError != null)
      return _errorBox("Couldn't load files.", state.filesError!);

    final speeches =
        state.files.where((f) => f['category'] == 'speech').toList();
    ...
    return ListView(
      ...
      children: [
        ...
        if (speeches.isEmpty)
          ...
          for (final f in speeches)
            _FileRow(
                file: f,
                icon: _iconFor(f['name'] as String?),
                onDownload: () => _download(f),
                onDelete: () => ref
                    .read(weddingPartyProvider.notifier)
                    .deleteFile(f['id'] as int)),
        const SizedBox(height: 8),
        OutlinedButton.icon(
          onPressed: _uploadingSpeech ? null : () => _upload('speech'),
          icon: _uploadingSpeech
              ? const SizedBox(
                  width: 14,
                  height: 14,
        ...
        child: Row(children: [
          Container(
              ...
              decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(10)),
              child: Icon(icon, color: AppTheme.udoGreen, size: 20)),
          const SizedBox(width: 12),
          Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                ...
                Text(file['name'] as String? ?? '',
                    ...
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis),
                Text(_formatFileSize(file['file_size_bytes'] as int?),
                    style: const TextStyle(
                        fontSize: 12, color: AppTheme.udoTextSecondary)),
              ])),
          GestureDetector(
...
class _PersonDetailSheet extends ConsumerStatefulWidget {
  ...
  const _PersonDetailSheet(
      {required this.person, required this.responsibilities});
  @override
...
class _PersonDetailSheetState extends ConsumerState<_PersonDetailSheet> {
  ...
  void initState() {
    ...
    _roleCtrl = TextEditingController(
        text: widget.person['wedding_party_role'] as String? ?? '');
    _attireStatus = widget.person['attire_status'] as String? ?? 'not_started';
    _rehearsalStatus =
        widget.person['rehearsal_status'] as String? ?? 'pending';
  }
  ...
  Future<void> _save() async {
    ...
    final ok = await ref
        .read(weddingPartyProvider.notifier)
        .updateMember(widget.person['id'] as int, {
    ...
    setState(() => _saving = false);
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(ok ? 'Saved.' : "Couldn't save. Try again.")));
  }
  ...
  Future<void> _remove() async {
    ...
    await ref
        .read(weddingPartyProvider.notifier)
        .removeMember(widget.person['id'] as int);
  }
            ...
            child: Row(children: [
              ...
              Expanded(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                    Text(_fullName(person),
                        style: const TextStyle(
                            fontSize: 18, fontWeight: FontWeight.w600)),
                    Text(
                        (person['wedding_party_role'] as String?)?.isNotEmpty ==
                                true
                            ? person['wedding_party_role'] as String
                            : 'Wedding party',
                        style: const TextStyle(
              ...
              child: SingleChildScrollView(
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        Wrap(spacing: 6, runSpacing: 6, children: [
                          for (final s in const [
                            ...
                            ChoiceChip(
                              ...
                              onSelected: (_) =>
                                  setState(() => _attireStatus = s),
                              selectedColor: AppTheme.udoGreen,
                              labelStyle: TextStyle(
                                  color: _attireStatus == s
                                      ? Colors.white
                                      : AppTheme.udoTextPrimary),
                              side: BorderSide(
                                  color: _attireStatus == s
                        ...
                        Wrap(spacing: 6, runSpacing: 6, children: [
                          for (final s in const [
                            ...
                            ChoiceChip(
                              ...
                              onSelected: (_) =>
                                  setState(() => _rehearsalStatus = s),
                              selectedColor: AppTheme.udoGreen,
                              labelStyle: TextStyle(
                                  color: _rehearsalStatus == s
                                      ? Colors.white
                                      : AppTheme.udoTextPrimary),
                              side: BorderSide(
                                  color: _rehearsalStatus == s
                        ...
                        if (widget.responsibilities.isEmpty)
                          ...
                          Container(
                            ...
                            child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  for (final r in widget.responsibilities)
                                    Padding(
                                      padding: const EdgeInsets.only(bottom: 6),
                                      child: Row(children: [
                                        Expanded(
                        ...
                        _InfoRow(
                            'Phone',
                            (person['phone'] as String?)?.isNotEmpty == true
                                ? person['phone'] as String
                                : 'Not on file'),
                        _InfoRow(
                            'Email',
                            (person['email'] as String?)?.isNotEmpty == true
                        ...
                        Row(children: [
                          Expanded(
                              child: OutlinedButton.icon(
                            onPressed: () =>
                                _launchTel(context, person['phone'] as String?),
                        ...
                        TextButton.icon(
                            ...
                            icon: const Icon(Icons.delete_outline,
                                size: 16, color: Colors.red),
                            label: const Text('Remove from wedding party',
...
class _LinkExistingGuestSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_LinkExistingGuestSheet> createState() =>
      _LinkExistingGuestSheetState();
}

class _LinkExistingGuestSheetState
    extends ConsumerState<_LinkExistingGuestSheet> {
      ...
      child: SafeArea(
        child: Column(children: [
          Padding(
            ...
            child: Row(children: [
              const Expanded(
                  child: Text('Select an existing guest',
                      style: TextStyle(
                          fontSize: 18, fontWeight: FontWeight.w600))),
              IconButton(
                  onPressed: () => Navigator.pop(context),
                ...
                : candidates.isEmpty
                    ? Center(
                        child: Padding(
                          padding: const EdgeInsets.all(24),
                          child: Text(
                            guestsState.guests.isEmpty
                                ? 'No guests yet. Add guests first, or add this person directly.'
                                : 'No matching guests, or everyone matching is already in your wedding party.',
                            textAlign: TextAlign.center,
                            style: const TextStyle(
                                fontSize: 13, color: AppTheme.udoTextSecondary),
                          ),
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items: const [
                            DropdownMenuItem(
                                value: 'Maid of honour',
                                child: Text('Maid of honour')),
                            DropdownMenuItem(
                                value: 'Best man', child: Text('Best man')),
                            DropdownMenuItem(
                                value: 'Bridesmaid', child: Text('Bridesmaid')),
                            DropdownMenuItem(
                                value: 'Groomsman', child: Text('Groomsman')),
                            DropdownMenuItem(
                                value: 'Parent', child: Text('Parent')),
                            DropdownMenuItem(
                                value: 'Flower girl',
                                child: Text('Flower girl')),
                            DropdownMenuItem(
                                value: 'Ring bearer',
                                child: Text('Ring bearer')),
                          ],
                          onChanged: (v) =>
                              setSheetState(() => role = v ?? role),
                        ),
                        const SizedBox(height: 16),
                        ElevatedButton(
                          onPressed: () async {
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: const [
                      ...
                      DropdownMenuItem(
                          value: 'Groomsman', child: Text('Groomsman')),
                      DropdownMenuItem(
                          value: 'Flower girl', child: Text('Flower girl')),
                      DropdownMenuItem(
                          value: 'Ring bearer', child: Text('Ring bearer')),
                    ],
                  ...
                  ElevatedButton(
                    ...
                    child: _saving
                        ? const SizedBox(
                            ...
                            child: CircularProgressIndicator(
                                strokeWidth: 2, color: Colors.white))
                        : const Text('Add to wedding party'),
                  ),
                ]),
          ),
        ),
